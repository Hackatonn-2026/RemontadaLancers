<template>
  <div class="cabecalho-cadastro">
    <h1>Criar conta</h1>
    <p>Encontre o profissional certo para seu projeto.</p>
  </div>
  <div class="tipo-usuario">
    <button
      type="button"
      :class="{ ativo: tipoUsuario === 'cliente' }"
    ...
    <button
      type="button"
      :class="{ ativo: tipoUsuario === 'freelancer' }"
      @click="tipoUsuario = 'freelancer'"
    >
      Sou profissional
    </button>
  </div>
  <div class="dados-cadastro">
    <form class="cadastro-form" @submit.prevent="CriaConta">
      ...
      <RouterLink to="/login" class="botao-login"> Faça login </RouterLink>
    </form>
  </div>
</template>
<script setup>
...
import { obterUsuariosCadastrados, salvarUsuarioAtual } from '@/data/usuarios'
...
function CriaConta() {
  ...
  const rota = tipoUsuario.value === 'freelancer' ? `/usuario-freelancer/${usuario.id}` : '/perfil'
